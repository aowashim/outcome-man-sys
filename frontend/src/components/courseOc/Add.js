...
import { addData } from '../../helpers/courseOcServer'
...
const Add = props => {
  ...
  const handleAdd = async e => {
    ...
    setIsLoading(true)
    const data = await addData(`${props.path}`, oc, ccode)
    ...
    <form onSubmit={handleAdd}>
      <div className='flex mt-5 bg-gray-400 p-3 justify-center mx-5 lg:mx-36 md:mx-20 rounded-3xl'>
        <div>
          ...
          <div className='mt-2'>
            <label>Enter c. code : </label>
            <input
              type='text'
              required
              className='border rounded-lg border-green-400 ml-6'
              value={ccode}
